//
//  DetailBLogDrug.swift
//  OBQI
//
//  Created by t.o on 2017/04/11.
//  Copyright © 2017年 System. All rights reserved.
//

import UIKit

class DetailBLogDrug: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        mySearchBar.placeholder = "医薬品名またはYJコードを入力してください"
        ...
        let url = "\(AppConst.URLPrefix)dictionary/GetDrugHistory/\(customerID)"
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if InputLabel.text != defaultLabel && indexPath != nil{
            ansArray = [searchResults[(indexPath! as NSIndexPath).row]["OrderMainCD"].asString!]
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = searchResults[index]["SalesName"].asString
        cell.detailTextLabel?.text = searchResults[index]["YJCode"].asString
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        InputLabel.text = "選択値：\(searchResults[index]["SalesName"].asString!)"
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        searchDrug(convertSearchText(searchText))
    }
    ...
    @IBAction func clickSearchMode(_ sender: UIButton) {
        ...
        // モードが変わったら再検索
        searchDrug(convertSearchText(mySearchBar.text!))
    }
    ...
    /*
     検索処理
     */
    private func searchDrug(_ searchText:String) {
        ...
        case SearchTarget.ALL: // 医薬品名+YJコード
            // 検索対象テーブル
            searchTable = appDelegate.ViewDrug
            // 検索対象カラム
            whereTargets = ["SalesName", "YJCode"]
            break

        case SearchTarget.NAME: // 医薬品名
            // 検索対象テーブル
            searchTable = appDelegate.ViewDrug
            // 検索対象カラム
            whereTargets = ["SalesName"]
            break

        case SearchTarget.CODE: // YJコード
            // 検索対象テーブル
            searchTable = appDelegate.ViewDrug
            // 検索対象カラム
            whereTargets = ["YJCode"]
...
extension DetailBLogDrug {
    /** キーボードのNotificationを購読開始 */
    func startObserveKeyboardNotification(){
        let center = NotificationCenter.default
       // center.addObserver(self, selector: #selector(type(of: self).willShowKeyboard(notification:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
      //  center.addObserver(self, selector: #selector(type(of: self).willHideKeyboard(notification:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
