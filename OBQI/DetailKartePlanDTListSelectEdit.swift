...
class DetailKartePlanDTListSelectEdit: UITableViewController, UIPopoverPresentationControllerDelegate {
    ...
    // 選択されたヘッダ
    var selectedHD:AppConst.KarteMenuHDParamsFormat?
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // 選択済みにする
        let DTCount = appDelegate.KartePlan?.currentSelectedData.filter{
    ...
    private func selectAction() -> ((Int, Int) -> Void) {
        return { (groupIndex: Int, itemIndex: Int) -> Void in
            // 選択済の場合はスキップ
            let isExist = self.appDelegate.KartePlan?.currentSelectedData.filter{
            ...
            if isExist == nil {
                self.appDelegate.KartePlan?.currentSelectedData.append(self.myItemsBySection[groupIndex][itemIndex]!)
            }
        }
    }
    ...
    private func deSelectAction() -> ((Int, Int) -> Void) {
        return { (groupIndex: Int, itemIndex: Int) -> Void in
            // 選択解除された値を除外
            let DTIndex = self.appDelegate.KartePlan?.currentSelectedData.enumerated().filter{
            ...
            if DTIndex != nil {
                self.appDelegate.KartePlan?.currentSelectedData.remove(at: DTIndex!)
            }
        }
    }
    ...
    @IBAction func ClickAllSelect(_ sender: UIBarButtonItem) {
        kcCommon.allSelect(tableView, action: selectAction())
    }
    
    // 一括解除
    @IBAction func ClickAllDeSelect(_ sender: UIBarButtonItem) {
        kcCommon.allDeSelect(tableView, action: deSelectAction())
    }
}
